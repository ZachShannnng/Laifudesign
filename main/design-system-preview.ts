...
<head>
  ...
  <title>${escapeHtml(title)} — 设计系统预览</title>
  <style>
    :root {
      --bg: ${bg};
      --fg: ${fg};
...
<body>
  <main class="wrap">
    <span class="badge">设计系统预览 · ${escapeHtml(id)}</span>
    ...
    <section>
      <h2 class="section-title">组件示例</h2>
...
function extractColors(raw: string): Array<{ name: string; value: string }> {
  ...
  // Form A: "- **Primary:** `#3B82F6`" / "- Background: #FAFAFA"
  // 更精确匹配：行首空白 + 可选的列表标记（- 或 *） + **Name:** + 可选空格 + `#HEX`
...
function extractFonts(raw: string): Record<string, string> {
...
function pickColor(colors: Array<{ name: string; value: string }>, hints: string[]): string | null {
...
function firstNonNeutral(colors: Array<{ name: string; value: string }>): string | null {
...
function parseAlignments(separatorLine: string, count: number): (string | null)[] {
...
function renderTable(header: string[], rows: string[][], aligns: (string | null)[]): string {
  const th = header
    .map((cell, k) => {
