---
description: 洁癖级知识库同步 — 会话结束后对项目文档(CLAUDE.md, README.md, docs/)和 agent 记忆进行审查与同步，确保知识体系跟得上代码变化。触发词：/neat, /sync, 同步一下, 整理文档, 整理一下, 更新记忆, 收尾
---
...
> Agent 记忆系统的具体位置因平台而异（Claude Code 在 `~/.claude/projects/<...>/memory/`，Codex 用 `AGENTS.md`，OpenCode 用 `.opencode/`，OpenClaw 用 `~/.openclaw/`）。完整路径速查见 [agent-paths.md](neat-freak/agent-paths.md)。如果当前 agent 没有独立的记忆系统，直接跳过这一层，把功夫全花在 docs 和项目根 markdown 上。
...
1. 列出 agent 的记忆文件（如有）：
   ...
   - Codex / OpenCode / 其他：找该 agent 的等价位置（见 neat-freak/agent-paths.md）
2. 对本次对话涉及的**每一个项目**：
...
完整映射表见 **[sync-matrix.md](neat-freak/sync-matrix.md)**——遇到不确定的改动先查这张表。
...
**跨项目改动**：本次对话改了多个项目，每个项目都要跑一次完整的第一步（ls + 读 docs）。

**发现之前的同步漏了东西**：修掉。不要说"那不是这次对话的事"——你就是这个项目的持续编辑，过去的漏洞也归你管。

## 参考资料

- **[sync-matrix.md](neat-freak/sync-matrix.md)** — 完整的"变更类型 → 要改哪些文件"映射表
- **[agent-paths.md](neat-freak/agent-paths.md)** — Claude Code / Codex / OpenCode 各自的记忆与配置路径速查
